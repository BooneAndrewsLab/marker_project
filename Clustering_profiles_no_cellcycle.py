...
parser.add_argument('-s', '--strain_path',
                    help='Path containing final list of strains to include in the clustering analysis after removing '
                         '26C, multiple alleles, DMA-TS, low cell count and low weighted penetrant strains.')
...
if __name__ == '__main__':
    ...
    # Get strains list to include and filter dataframes
    strains_to_include = pd.read_csv(args.strain_path)['Strain'].str.lower()
    df_data = df_data[df_data['Strain ID'].str.lower().isin(strains_to_include)]
    df_outliers = df_outliers[df_outliers['Strain ID'].str.lower().isin(strains_to_include)]
